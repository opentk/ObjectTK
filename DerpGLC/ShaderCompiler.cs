...
namespace DerpGLC
{
    ...
    /// </summary>
    /// <remarks>
    /// The easiest way to use this feature is to add a reference to DerpGLC so that the executable gets copied to the output folder and add the following Post-build event:<br/>
    /// "$(TargetDir)DerpGLC.exe" "$(TargetPath)"
    /// </remarks>
